...
void MemorySystem::Impl::Initialize(
    ...
    const std::optional<DispatcherParameters>& dispatcher_parameters) {
  LOG(INFO) << "[MemorySystem] Initialize enter gwp="
            << static_cast<bool>(gwp_asan_parameters) << " profiling="
            << static_cast<bool>(profiling_client_parameters) << " dispatcher="
            << static_cast<bool>(dispatcher_parameters);
  if (!IsAllocatorShimInitialized()) {
    LOG(INFO) << "[MemorySystem] Allocator shim not initialized";
    return;
  }
...
void MemorySystem::Impl::InitializeGwpASan(
    ...
    InitializationData& initialization_data) {
  LOG(INFO) << "[MemorySystem] InitializeGwpASan enter boost="
            << gwp_asan_parameters.boost_sampling << " process_type="
            << gwp_asan_parameters.process_type;
#if BUILDFLAG(ENABLE_GWP_ASAN)
  LOG(INFO) << "[MemorySystem] MaybeEnableExtremeLightweightDetector";
