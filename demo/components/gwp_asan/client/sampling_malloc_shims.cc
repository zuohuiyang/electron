...
bool InstallMallocHooks(const AllocatorSettings& settings,
  ...
  LOG(INFO) << "[GWP-ASan][MallocHooks] Enter InstallMallocHooks "
  ...
  LOG(INFO) << "[GWP-ASan][MallocHooks] GPA created ptr=" << static_cast<void*>(gpa);

  if (!gpa->Init(settings, std::move(callback), false)) {
    LOG(INFO) << "[GWP-ASan][MallocHooks] GPA Init failed";
    return false;
  }
  LOG(INFO) << "[GWP-ASan][MallocHooks] GPA Init success crash_key=" << gpa->GetCrashKey();

  malloc_crash_key.Set(gpa->GetCrashKey());
  LOG(INFO) << "[GWP-ASan][MallocHooks] CrashKey set";

  sampling_state.Init(settings.sampling_frequency);
  LOG(INFO) << "[GWP-ASan][MallocHooks] SamplingState.Init freq=" << settings.sampling_frequency;

  sampling_state.SetSampleSizeRestriction(settings.sampling_min_size,
                                          settings.sampling_max_size);
  LOG(INFO) << "[GWP-ASan][MallocHooks] SamplingState.SetSampleSizeRestriction min="
            << settings.sampling_min_size << " max=" << settings.sampling_max_size;

  allocator_shim::InsertAllocatorDispatch(&g_allocator_dispatch);
  //LOG(INFO) << "[GWP-ASan][MallocHooks] Allocator shim dispatch inserted";
  //安装钩子后不能打日志，可能陷入死循环
  return true;
}
